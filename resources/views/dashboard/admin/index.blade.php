...
<div class="col-md-10 p-0">
    <h2 class="content-title text-center">Daftar Security</h2>
    ...
    <table class="table table-hover table-stripped table-bordered text-center dt-head-center" id="datatable">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{$sec->name}} </td>
          <td>{{$sec->nomor_induk}} </td>
          <td>{{$sec->email}} </td>
          <td style="font-size: 22px;">
            <a href="/dashboard/users/{{ $sec->id }}/edit" class="edituser" data-id="{{ $sec->id }}" data-bs-toggle="modal" data-bs-target="#edituser"><i class="bi bi-pencil-square text-warning"></i></a>&nbsp;
            <a href="/dashboard/admin/{{ $sec->id }}/removeSecurity" class="bi bi-trash-fill text-danger border-0" onclick="return confirm('Ubah security menjadi loaner?')"></a>
          </td>
        </tr>
        @endforeach
      </tbody>
